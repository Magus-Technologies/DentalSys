...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($ap === 'crear') {
        ...
        $stNC = db()->prepare("SELECT id FROM notas_credito WHERE pago_id=? AND tipo_nota='credito' AND sunat_estado='aceptado' AND estado != 'anulada' LIMIT 1");
    ...
    if ($ap === 'anular_nota_credito') {
        $nota_ref_id = (int)($_POST['nota_ref_id'] ?? 0);
        $cod_motivo  = trim($_POST['cod_motivo']  ?? '01');
        $des_motivo  = trim($_POST['des_motivo']  ?? 'Anulación de nota de crédito emitida con error.');

        $stRef = db()->prepare("SELECT * FROM notas_credito WHERE id=?");
        $stRef->execute([$nota_ref_id]);
        $notaRef = $stRef->fetch();

        if (!$notaRef || $notaRef['sunat_estado'] !== 'aceptado' || $notaRef['estado'] === 'anulada') {
            flash('error', 'La nota no puede anularse (no existe, no está aceptada o ya fue anulada).');
            go("pages/notas_credito.php?accion=ver&id=$nota_ref_id");
        }

        $stEx = db()->prepare("SELECT id FROM notas_credito WHERE nota_ref_id=? AND sunat_estado != 'rechazado' LIMIT 1");
        $stEx->execute([$nota_ref_id]);
        if ($stEx->fetch()) {
            flash('error', 'Ya existe una nota de corrección activa para esta nota de crédito.');
            go("pages/notas_credito.php?accion=ver&id=$nota_ref_id");
        }
        ...
        $ins = db()->prepare("
            INSERT INTO notas_credito(pago_id, nota_ref_id, tipo_nota, serie, numero, cod_motivo, des_motivo, total, aplica_igv)
            VALUES(?, ?, 'credito', ?, ?, ?, ?, ?, ?)
        ");
        $ins->execute([
            $notaRef['pago_id'],
            ...
            $des_motivo,
            (float)$notaRef['total'],
            (int)($notaRef['aplica_igv'] ?? 0),
        ]);
        $nuevaNotaId = (int)db()->lastInsertId();
        auditar('CREAR_NC_DE_NC', 'notas_credito', $nuevaNotaId);
        flash('ok', 'Nota de corrección creada. Generá el XML y enviala a SUNAT.');
        go("pages/notas_credito.php?accion=ver&id=$nuevaNotaId");
    }

    if ($ap === 'baja_resumen') {
        set_time_limit(120);
        $nid = (int)$_POST['id'];
        $r   = (new SunatService(db()))->darDeBajaNotaResumen($nid);
        if ($r['ok']) {
            flash('ok', 'SUNAT aceptó la baja. ' . $r['mensaje']);
        } else {
            flash('error', 'Error: ' . $r['mensaje']);
        }
        go("pages/notas_credito.php?accion=ver&id=$nid");
    }
    ...
    if ($ap === 'dar_de_baja') {
        ...
        if ($r['ok']) {
            ...
            flash('warning', $r['mensaje']);
        } else {
            flash('error', 'Error al dar de baja: ' . $r['mensaje']);
        }
        go("pages/notas_credito.php?accion=ver&id=$nid");
    }
}
...
<div class="card mt-2"><div class="table-responsive"><table class="table mb-0">
 ...
 <?php foreach ($lista as $n):
  ...
  $sc = $anulada ? 'br' : ($se==='aceptado'?'bg':($se==='rechazado'?'br':($se==='pendiente'?'ba':'bgr')));
 ...
 ?>
  <tr>
   ...
   <td style="font-size:12px;white-space:nowrap"><?=fDate($n['created_at'])?></td>
   <td><span class="badge <?=$sc?>"><?=$anulada?'ANULADA':($se?strtoupper($se):'SIN XML')?></span></td>
   <td>
 ...
 <?php endforeach; if (!$lista): ?>
  <tr><td colspan="9" class="text-center py-4" style="color:var(--t2)">
 ...
 <div class="col-12 col-lg-7">
  <div class="card">
   ...
   <div class="p-4">
    ...
    <?php if (!empty($nota['nota_ref_id'])): ?>
    <div class="mb-3 p-2 rounded d-flex align-items-center gap-2" style="background:rgba(255,193,7,.1);border:1px solid rgba(255,193,7,.4);font-size:11px">
     <i class="bi bi-arrow-return-right" style="color:#ffc107"></i>
     <span>Esta es una <strong>nota de corrección</strong>. Anula la nota
      <a href="?accion=ver&id=<?=$nota['nota_ref_id']?>">NC #<?=$nota['nota_ref_id']?></a> ante SUNAT.
     </span>
    </div>
    <?php endif; ?>
   ...
   <div class="p-3">
    ...
    <div class="d-grid gap-2">
     ...
     <?php if ($nota['estado'] === 'anulada'): ?>
      <div class="mt-2 p-2 rounded text-center" style="background:var(--bg3);font-size:11px;color:var(--t2)">Esta nota fue anulada ante SUNAT.</div>
     <?php elseif ($nota['tipo_nota'] === 'credito' && $se === 'aceptado'): ?>
      <?php
        $stNcNc = db()->prepare("SELECT id FROM notas_credito WHERE nota_ref_id=? AND sunat_estado != 'rechazado' LIMIT 1");
        $stNcNc->execute([$id]);
        $ncNcExiste = $stNcNc->fetch();
      ?>
      <?php if ($ncNcExiste): ?>
       <div class="mt-2 p-2 rounded text-center" style="background:var(--bg3);font-size:11px;color:var(--t2)">
        <i class="bi bi-info-circle me-1"></i>Nota de corrección emitida:
        <a href="?accion=ver&id=<?=$ncNcExiste['id']?>">ver</a>
       </div>
      <?php else: ?>
       <?php
         $diasDesdeEmision = (int)(new DateTime('today'))->diff(new DateTime($nota['created_at']))->days;
         $dentroDelPlazo   = $diasDesdeEmision <= 7;
       ?>
       <?php if ($dentroDelPlazo): ?>
       <div class="mt-3 p-3 rounded" style="border:1px solid rgba(220,53,69,.4);background:rgba(220,53,69,.05)">
        <div class="mb-2" style="font-size:11px;color:var(--t2)">
         <i class="bi bi-exclamation-triangle me-1"></i>
         <strong>¿Esta nota tiene un error?</strong>
         Anulala via Resumen Diario (plazo: 7 días — quedan <?=7-$diasDesdeEmision?> día(s)).
        </div>
        <form method="POST" onsubmit="return confirm('¿Anular esta nota via Resumen Diario? SUNAT la marcará como anulada.')">
         <input type="hidden" name="accion" value="baja_resumen">
         <input type="hidden" name="id" value="<?=$id?>">
         <button type="submit" class="btn btn-sm w-100" style="background:#dc3545;color:#fff;border:none">
          <i class="bi bi-x-circle me-1"></i>Anular via Resumen Diario
         </button>
        </form>
       </div>
       <?php else: ?>
       <div class="mt-2 p-2 rounded text-center" style="background:var(--bg3);font-size:11px;color:var(--t2)">
        <i class="bi bi-lock me-1"></i>Fuera del plazo de 7 días — no se puede anular.
       </div>
       <?php endif; ?>
      <?php endif; ?>
     <?php endif; ?>
    ...
    $pagos = db()->query("
        ...
        WHERE p.tipo_comprobante IN('boleta','factura')
          ...
          AND NOT EXISTS (
              ...
              WHERE nc.pago_id = p.id AND nc.tipo_nota='credito' AND nc.sunat_estado='aceptado' AND nc.estado != 'anulada'
          )
